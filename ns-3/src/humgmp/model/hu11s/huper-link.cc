...
HuperLink::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::hu11s::HuperLink")
    ...
    .AddAttribute ( "RetryTimeout",
                    ...
                    MakeTimeAccessor (
                      &HuperLink::m_dot11MeshnetRetryTimeout),
                    MakeTimeChecker ()
                    )
    .AddAttribute ( "HoldingTimeout",
                    ...
                    TimeValue (TimeValue (MicroSeconds (40 * 1024))),
                    MakeTimeAccessor (
                      &HuperLink::m_dot11MeshnetHoldingTimeout),
                    MakeTimeChecker ()
                    )
    .AddAttribute ( "ConfirmTimeout",
                    ...
                    TimeValue (TimeValue (MicroSeconds (40 * 1024))),
                    MakeTimeAccessor (
                      &HuperLink::m_dot11MeshnetConfirmTimeout),
                    MakeTimeChecker ()
                    )
    .AddAttribute ( "MaxRetries",
                    ...
                    MakeUintegerAccessor (
                      &HuperLink::m_dot11MeshnetMaxRetries),
...
HuperLink::HuperLink () :
  ...
  m_huperMeshnetPointAddress (Mac48Address::GetBroadcast ()),
  m_localLinkId (0),
  m_huperLinkId (0),
  m_assocId (0),
  m_huperAssocId (0),
...
void
HuperLink::SetHuperMeshnetPointAddress (Mac48Address macaddr)
{
  m_huperMeshnetPointAddress = macaddr;
}
void
...
HuperLink::OpenAccept (uint16_t localLinkId, IeConfiguration conf, Mac48Address huperMp)
{
  ...
  if (m_huperMeshnetPointAddress != Mac48Address::GetBroadcast ())
    {
      NS_ASSERT (m_huperMeshnetPointAddress == huperMp);
    }
  else
    {
      m_huperMeshnetPointAddress = huperMp;
    }
  ...
  switch (m_state)
    ...
    case IDLE:
      switch (event)
        ...
        case CLS_ACPT:
          m_state = IDLE;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, IDLE, IDLE);
          break;
        case REQ_RJCT:
        ...
        case ACTOPN:
          m_state = OPN_SNT;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, IDLE, OPN_SNT);
          SendHuperLinkOpen ();
          SetRetryTimer ();
          break;
        case OPN_ACPT:
          m_state = OPN_RCVD;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, IDLE, OPN_RCVD);
    ...
    case OPN_SNT:
      switch (event)
        ...
        case CNF_ACPT:
          m_state = CNF_RCVD;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, OPN_SNT, CNF_RCVD);
          ClearRetryTimer ();
          SetConfirmTimer ();
          break;
        case OPN_ACPT:
          m_state = OPN_RCVD;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, OPN_SNT, OPN_RCVD);
          SendHuperLinkConfirm ();
          break;
        case CLS_ACPT:
          m_state = HOLDING;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, OPN_SNT, HOLDING);
          ClearRetryTimer ();
          SendHuperLinkClose (REASON11S_MESHNET_CLOSE_RCVD);
          SetHoldingTimer ();
          break;
        case OPN_RJCT:
        case CNF_RJCT:
        ...
        case TOR2:
          ...
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, OPN_SNT, HOLDING);
          ClearRetryTimer ();
          SendHuperLinkClose (REASON11S_MESHNET_MAX_RETRIES);
          SetHoldingTimer ();
          break;
        case CNCL:
          m_state = HOLDING;
    ...
    case CNF_RCVD:
      switch (event)
        ...
        case OPN_ACPT:
          m_state = ESTAB;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, CNF_RCVD, ESTAB);
          ClearConfirmTimer ();
          SendHuperLinkConfirm ();
          NS_ASSERT (m_huperMeshnetPointAddress != Mac48Address::GetBroadcast ());
          break;
        case CLS_ACPT:
          m_state = HOLDING;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, CNF_RCVD, HOLDING);
          ClearConfirmTimer ();
          SendHuperLinkClose (REASON11S_MESHNET_CLOSE_RCVD);
          SetHoldingTimer ();
          break;
        ...
        case TOC:
          m_state = HOLDING;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, CNF_RCVD, HOLDING);
          SendHuperLinkClose (REASON11S_MESHNET_CONFIRM_TIMEOUT);
          SetHoldingTimer ();
          break;
        default:
    ...
    case OPN_RCVD:
      switch (event)
        ...
        case CNF_ACPT:
          m_state = ESTAB;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, OPN_RCVD, ESTAB);
          ClearRetryTimer ();
          NS_ASSERT (m_huperMeshnetPointAddress != Mac48Address::GetBroadcast ());
          break;
        case CLS_ACPT:
          m_state = HOLDING;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, OPN_RCVD, HOLDING);
          ClearRetryTimer ();
          SendHuperLinkClose (REASON11S_MESHNET_CLOSE_RCVD);
          SetHoldingTimer ();
          break;
    ...
    case ESTAB:
      switch (event)
        ...
        case CLS_ACPT:
          m_state = HOLDING;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, ESTAB, HOLDING);
          SendHuperLinkClose (REASON11S_MESHNET_CLOSE_RCVD);
          SetHoldingTimer ();
          break;
        case OPN_RJCT:
    ...
    case HOLDING:
      switch (event)
        ...
        case TOH:
          m_state = IDLE;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, HOLDING, IDLE);
          break;
        case OPN_ACPT:
        case CNF_ACPT:
          m_state = HOLDING;
          m_linkStatusCallback (m_interface, m_huperAddress, m_huperMeshnetPointAddress, HOLDING, HOLDING);
...
HuperLink::SendHuperLinkClose (PmpReasonCode reasoncode)
{
  ...
  m_macPlugin->SendHuperLinkManagementFrame (m_huperAddress, m_huperMeshnetPointAddress, m_assocId, huperElement,
                                            m_configuration);
}
void
...
{
  NS_ASSERT (m_dot11MeshnetHoldingTimeout.GetMicroSeconds () != 0);
  m_holdingTimer = Simulator::Schedule (m_dot11MeshnetHoldingTimeout, &HuperLink::HoldingTimeout, this);
}
void
...
{
  NS_ASSERT (m_dot11MeshnetRetryTimeout.GetMicroSeconds () != 0);
  m_retryTimer = Simulator::Schedule (m_dot11MeshnetRetryTimeout, &HuperLink::RetryTimeout, this);
}
void
HuperLink::RetryTimeout ()
{
  if (m_retryCounter < m_dot11MeshnetMaxRetries)
    {
      StateMachine (TOR1);
    }
  else
...
{
  NS_ASSERT (m_dot11MeshnetConfirmTimeout.GetMicroSeconds () != 0);
  m_confirmTimer = Simulator::Schedule (m_dot11MeshnetConfirmTimeout, &HuperLink::ConfirmTimeout, this);
}
void
...
HuperLink::Report (std::ostream & os) const
{
  ...
  "huperMeshnetPointAddress=\"" << m_huperMeshnetPointAddress << "\"" << std::endl <<
