 ...
 * \ingroup hu11s
 *
 * \brief This is plugin to Meshnet WiFi MAC, which implements
 ...
 * element and meshnet configuration element and passes it to main part
 * of protocol
 */
class HuperManagementProtocolMac : public MeshnetWifiInterfaceMacPlugin
{
public:
  ...
  void SetParent (Ptr<MeshnetWifiInterfaceMac> parent);
  bool Receive (Ptr<Packet> packet, const WifiMacHeader & header);
  bool UpdateOutcomingFrame (Ptr<Packet> packet, WifiMacHeader & header, Mac48Address from, Mac48Address to);
  void UpdateBeacon (MeshnetWifiBeacon & beacon) const;
...
private:
  ...
  // \{
  Ptr<MeshnetWifiInterfaceMac> m_parent;
  uint32_t m_ifIndex;
