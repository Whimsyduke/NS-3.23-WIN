...
#include "ns3/meshnet-wifi-interface-mac-plugin.h"
...
bool
HuperManagementProtocol::Install (Ptr<MeshnetPointDevice> mp)
{
...
HuperManagementProtocol::ReceiveHuperLinkFrame (uint32_t interface, Mac48Address huperAddress,
                                              Mac48Address huperMeshnetPointAddress, uint16_t aid, IeHuperManagement huperManagementElement,
                                              IeConfiguration meshnetConfig)
{
  Ptr<HuperLink> huperLink = FindHuperLink (interface, huperAddress);
  if (huperManagementElement.SubtypeIsOpen ())
    {
      ...
      if (huperLink == 0)
        {
          huperLink = InitiateLink (interface, huperAddress, huperMeshnetPointAddress);
        }
      if (!reject)
        {
          huperLink->OpenAccept (huperManagementElement.GetLocalLinkId (), meshnetConfig, huperMeshnetPointAddress);
        }
      else
        {
          huperLink->OpenReject (huperManagementElement.GetLocalLinkId (), meshnetConfig, huperMeshnetPointAddress,
                                reasonCode);
        }
    }
  if (huperLink == 0)
  ...
  if (huperManagementElement.SubtypeIsConfirm ())
    {
      huperLink->ConfirmAccept (huperManagementElement.GetLocalLinkId (),
                               huperManagementElement.GetHuperLinkId (), aid, meshnetConfig, huperMeshnetPointAddress);
    }
...
HuperManagementProtocol::ConfigurationMismatch (uint32_t interface, Mac48Address huperAddress)
{
  ...
  if (huperLink != 0)
    {
      huperLink->MLMECancelHuperLink (REASON11S_MESHNET_CAPABILITY_POLICY_VIOLATION);
    }
}
void
...
HuperManagementProtocol::InitiateLink (uint32_t interface, Mac48Address huperAddress,
                                      Mac48Address huperMeshnetPointAddress)
{
  ...
  new_link->SetHuperMeshnetPointAddress (huperMeshnetPointAddress);
                                          ...
                                          PmpReasonCode & reasonCode)
{
  if (m_stats.linksTotal > m_maxNumberOfHuperLinks)
    {
      reasonCode = REASON11S_MESHNET_MAX_HUPERS;
      return false;
    }
  return true;
}
...
HuperManagementProtocol::HuperLinkStatus (uint32_t interface, Mac48Address huperAddress,
                                        Mac48Address huperMeshnetPointAddress, HuperLink::HuperState ostate, HuperLink::HuperState nstate)
{
  HuperManagementProtocolMacMap::iterator plugin = m_plugins.find (interface);
  NS_ASSERT (plugin != m_plugins.end ());
  NS_LOG_DEBUG ("Link between me:" << m_address << " my interface:" << plugin->second->GetAddress ()
                                   << " and huper meshnet point:" << huperMeshnetPointAddress << " and its interface:" << huperAddress
                                   << ", at my interface ID:" << interface << ". State movement:" << ostate << " -> " << nstate);
  if ((nstate == HuperLink::ESTAB) && (ostate != HuperLink::ESTAB))
    {
      NotifyLinkOpen (huperMeshnetPointAddress, huperAddress, plugin->second->GetAddress (), interface);
    }
  if ((ostate == HuperLink::ESTAB) && (nstate != HuperLink::ESTAB))
    {
      NotifyLinkClose (huperMeshnetPointAddress, huperAddress, plugin->second->GetAddress (), interface);
    }
...
Ptr<IeMeshnetId>
HuperManagementProtocol::GetMeshnetId () const
{
  NS_ASSERT (m_meshnetId != 0);
  return m_meshnetId;
}
void
HuperManagementProtocol::SetMeshnetId (std::string s)
{
  m_meshnetId = Create<IeMeshnetId> (s);
}
Mac48Address
