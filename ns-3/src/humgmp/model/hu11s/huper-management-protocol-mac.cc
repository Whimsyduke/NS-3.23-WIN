...
void
HuperManagementProtocolMac::SetParent (Ptr<MeshnetWifiInterfaceMac> parent)
{
  m_parent = parent;
  ...
  if (header.IsBeacon ())
    {
      ...
      Ptr<IeMeshnetId> meshnetId = DynamicCast<IeMeshnetId> (elements.FindFirst (IE11S_MESHNET_ID));

      if ((meshnetId != 0) && (m_protocol->GetMeshnetId ()->IsEqual (*meshnetId)))
        {
  ...
  if (header.IsAction ())
    {
     ...
     if ((actionValue.selfProtectedAction != WifiActionHeader::PEER_LINK_CONFIRM) && !fields.meshnetId.IsEqual (
            *(m_protocol->GetMeshnetId ())))
        {
          m_protocol->ConfigurationMismatch (m_ifIndex, huperAddress);
          // Broken huper link frame - drop it
          m_stats.brokenMgt++;
      ...
      //Huper Management element is the last element in this frame - so, we can use MeshnetInformationElementVector
      MeshnetInformationElementVector elements;
      packet->RemoveHeader (elements);
...
void
HuperManagementProtocolMac::UpdateBeacon (MeshnetWifiBeacon & beacon) const
{
  ...
  beacon.AddInformationElement (m_protocol->GetMeshnetId ());
...
HuperManagementProtocolMac::SendHuperLinkManagementFrame (Mac48Address huperAddress, Mac48Address huperMpAddress,
                                                        uint16_t aid, IeHuperManagement huperElement, IeConfiguration meshnetConfig)
{
  //Create a packet:
  meshnetConfig.SetNeighborCount (m_protocol->GetNumberOfLinks ());
  ...
  fields.capability = 0;
  fields.meshnetId = *(m_protocol->GetMeshnetId ());
  fields.config = meshnetConfig;
