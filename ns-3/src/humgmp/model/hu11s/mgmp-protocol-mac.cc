...
namespace ns3 {
	...
	namespace hu11s {
		...
		void
			MgmpProtocolMac::SetParent(Ptr<MeshnetWifiInterfaceMac> parent)
		{
			m_parent = parent;
		}
			...
			MgmpProtocolMac::ReceiveData(Ptr<Packet> packet, const WifiMacHeader & header)
		{
			...
			packet->RemoveHeader(meshnetHdr);
			m_stats.rxData++;
			...
			Mac48Address source;
			switch (meshnetHdr.GetAddressExt())
			{
			case 0:
			...
			tag.SetSeqno(meshnetHdr.GetMeshnetSeqno());
			tag.SetTtl(meshnetHdr.GetMeshnetTtl());
			packet->AddPacketTag(tag);

			if ((destination == Mac48Address::GetBroadcast()) && (m_protocol->DropDataFrame(meshnetHdr.GetMeshnetSeqno(),
				source)))
			{
				return false;
			}
			...
			MgmpProtocolMac::ReceiveAction(Ptr<Packet> packet, const WifiMacHeader & header)
		{
			...
			MeshnetInformationElementVector elements;
			packet->RemoveHeader(elements);
			std::vector<MgmpProtocol::FailedDestination> failedDestinations;
			for (MeshnetInformationElementVector::Iterator i = elements.Begin(); i != elements.End(); i++)
			{
				...
				if ((*i)->ElementId() == IE11S_RANN)
				{
					...
					NS_ASSERT(rann != 0);
					m_stats.rxRann++;
					if (rann->GetOriginatorAddress() == m_protocol->GetAddress())
					{
						continue;
					}
					if (rann->GetTtl() == 0)
				...
				Mac48Address to)
		{
			...
			MeshnetHeader meshnetHdr;
			meshnetHdr.SetMeshnetSeqno(tag.GetSeqno());
			meshnetHdr.SetMeshnetTtl(tag.GetTtl());
			packet->AddHeader(meshnetHdr);
...
#ifndef HUMGMP_UNUSED_MY_CODE
		void MgmpProtocolMac::SendRann(IeRann rann)
		{
			NS_LOG_FUNCTION_NOARGS();
			std::vector<IeRann> rann_vector;
			rann_vector.push_back(rann);
			SendRann(rann_vector);
		}
		void MgmpProtocolMac::SendRann(std::vector<IeRann> rann)
		{
			Ptr<Packet> packet = Create<Packet>();
			MeshnetInformationElementVector elements;
			for (std::vector<IeRann>::iterator i = rann.begin(); i != rann.end(); i++)
			{
				elements.AddInformationElement(Ptr<IeRann>(&(*i)));
			}
			packet->AddHeader(elements);
			packet->AddHeader(GetWifiActionHeader());
			//create 802.11 header:
			...
			hdr.SetAddr3(m_protocol->GetAddress());
			//Send Management frame
			std::vector<Mac48Address> receivers = m_protocol->GetRannReceivers(m_ifIndex);
			for (std::vector<Mac48Address>::const_iterator i = receivers.begin(); i != receivers.end(); i++)
			{
				hdr.SetAddr1(*i);
				m_stats.txRann++;
