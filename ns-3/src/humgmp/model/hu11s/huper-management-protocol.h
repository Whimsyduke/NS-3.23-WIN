...
public:
  ...
  void DoDispose ();
  /**
   * \brief Install PMP on given meshnet point.
   *
   * Installing protocol cause installing its interface MAC plugins.
   *
   * Also MP aggregates all installed protocols, PMP protocol can be accessed
   * via MeshnetPointDevice::GetObject<HuperManagementProtocol>();
   */
   ...
   * \param huperMeshnetPointAddress is address of huper meshnet point device (equal
   ...
   * \param meshnetConfig is meshnet configuration element taken from the huper
   * management frame
   */
  ...
  /**
   * \brief Cancels huper link due to broken configuration (Meshnet ID or Supported
   * rates)
   */
  ...
  Ptr<IeMeshnetId> GetMeshnetId () const;
...
private:
   ...
   */
  void HuperLinkStatus (uint32_t interface, Mac48Address huperAddress, Mac48Address huperMeshnetPointAddres, HuperLink::HuperState ostate, HuperLink::HuperState nstate);
  ///\brief BCA
...
private:
   ...
   * Mac48Address is huper address of meshnet point,
