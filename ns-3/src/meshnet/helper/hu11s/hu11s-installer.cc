...
Hu11sStack::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::Hu11sStack")
    ...
    .AddAttribute ("Root", 
                   "The MAC address of root meshnet point.",
...
void
Hu11sStack::Report (const Ptr<MeshnetPointDevice> mp, std::ostream& os)
{
  mp->Report (os);
  ...
  for (std::vector<Ptr<NetDevice> >::const_iterator i = ifaces.begin (); i != ifaces.end (); ++i)
    {
      ...
      NS_ASSERT (device != 0);
      Ptr<MeshnetWifiInterfaceMac> mac = device->GetMac ()->GetObject<MeshnetWifiInterfaceMac> ();
      NS_ASSERT (mac != 0);
      mac->Report (os);
    }
...
void
Hu11sStack::ResetStats (const Ptr<MeshnetPointDevice> mp)
{
  mp->ResetStats ();
