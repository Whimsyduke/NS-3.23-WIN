...
#ifndef MGMP_L2ROUTING_NET_DEVICE_H
#define MGMP_L2ROUTING_NET_DEVICE_H
 ...
 * \ingroup meshnet
 *
 * \brief Virtual net device modeling meshnet point.
 *
 * Meshnet point is a virtual net device which is responsible for
 *   - Aggregating and coordinating 1..* real devices -- meshnet interfaces, see MeshnetInterfaceDevice class.
 *   - Hosting all meshnet-related level 2 protocols.
 *
 * One of hosted L2 protocols must inplement L2RoutingProtocol interface and is used for packets forwarding.
 *
 * From the level 3 point of view MeshnetPointDevice is similar to BridgeNetDevice, but the packets,
 * which going through may be changed (because L2 protocols may require their own headers or tags).
 *
 * Attributes: \todo
 */
class MeshnetPointDevice : public NetDevice
{
public:
  /// Object type ID for NS3 object system
  static TypeId GetTypeId ();
  /// C-tor create empty (without interfaces and protocols) meshnet point
  MeshnetPointDevice ();
  /// D-tor
  virtual ~MeshnetPointDevice ();
   ...
   *
   * \attention Only MeshnetPointDevice can have IP address, but not individual interfaces.
   */
  ...
  ///\name Protocols
  //\{
  /// Register routing protocol to be used. Protocol must be already installed on this meshnet point.
  void SetRoutingProtocol (Ptr<MeshnetL2RoutingProtocol> protocol);
  /// Access current routing protocol
  Ptr<MeshnetL2RoutingProtocol> GetRoutingProtocol () const;
  //\}
...
private:
  ...
  /// Meshnet point MAC address, supposed to be the address of the first added interface
  ...
  Ptr<MeshnetL2RoutingProtocol> m_routingProtocol;
