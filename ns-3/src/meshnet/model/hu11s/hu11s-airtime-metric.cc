...
{
  static TypeId tid = TypeId ("ns3::hu11s::AirtimeLinkMetricCalculator")
    .SetParent<Object> ()
...
{
  m_testFrame = Create<Packet> (testLength + 6 /*Meshnet header*/ + 36 /*802.11 header*/);
}
uint32_t
AirtimeLinkMetricCalculator::CalculateMetric (Mac48Address huperAddress, Ptr<MeshnetWifiInterfaceMac> mac)
{
   ...
   */
  NS_ASSERT (!huperAddress.IsGroup ());
  //obtain current rate:
  WifiMode mode = mac->GetWifiRemoteStationManager ()->GetDataTxVector (huperAddress, &m_testHeader, m_testFrame, m_testFrame->GetSize ()).GetMode();
  //obtain frame error rate:
  double failAvg = mac->GetWifiRemoteStationManager ()->GetInfo (huperAddress).GetFrameErrorRate ();
  if (failAvg == 1)
    {
