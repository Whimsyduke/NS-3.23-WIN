	...
	namespace hu11s {
			...
			MgmpProtocol::GetTypeId()
		{
			static TypeId tid = TypeId("ns3::hu11s::MgmpProtocol")
				.SetParent<MeshnetL2RoutingProtocol>()
				...
				.AddAttribute("Hu11MeshnetMGMPmaxPREQretries",
					"Maximum number of retries before we suppose the destination to be unreachable",
					UintegerValue(3),
					MakeUintegerAccessor(
						&MgmpProtocol::m_hu11MeshnetMGMPmaxPREQretries),
					MakeUintegerChecker<uint8_t>(1)
					)
				.AddAttribute("Hu11MeshnetMGMPnetDiameterTraversalTime",
					"Time we suppose the packet to go from one edge of the network to another",
					TimeValue(MicroSeconds(1024 * 100)),
					MakeTimeAccessor(
						&MgmpProtocol::m_hu11MeshnetMGMPnetDiameterTraversalTime),
					MakeTimeChecker()
					)
				.AddAttribute("Hu11MeshnetMGMPpreqMinInterval",
					"Minimal interval between to successive PREQs",
					TimeValue(MicroSeconds(1024 * 100)),
					MakeTimeAccessor(
						&MgmpProtocol::m_hu11MeshnetMGMPpreqMinInterval),
					MakeTimeChecker()
					)
				.AddAttribute("Hu11MeshnetMGMPperrMinInterval",
					"Minimal interval between to successive PREQs",
					TimeValue(MicroSeconds(1024 * 100)),
					MakeTimeAccessor(&MgmpProtocol::m_hu11MeshnetMGMPperrMinInterval),
					MakeTimeChecker()
					)
				.AddAttribute("Hu11MeshnetMGMPactiveRootTimeout",
					"Lifetime of poractive routing information",
					TimeValue(MicroSeconds(1024 * 5000)),
					MakeTimeAccessor(
						&MgmpProtocol::m_hu11MeshnetMGMPactiveRootTimeout),
					MakeTimeChecker()
					)
				.AddAttribute("Hu11MeshnetMGMPactivePathTimeout",
					"Lifetime of reactive routing information",
					TimeValue(MicroSeconds(1024 * 5000)),
					MakeTimeAccessor(
						&MgmpProtocol::m_hu11MeshnetMGMPactivePathTimeout),
					MakeTimeChecker()
					)
				.AddAttribute("Hu11MeshnetMGMPpathToRootInterval",
					"Interval between two successive proactive PREQs",
					TimeValue(MicroSeconds(1024 * 2000)),
					MakeTimeAccessor(
						&MgmpProtocol::m_hu11MeshnetMGMPpathToRootInterval),
					MakeTimeChecker()
					)
				.AddAttribute("Hu11MeshnetMGMPrannInterval",
					"Lifetime of poractive routing information",
					TimeValue(MicroSeconds(1024 * 5000)),
					MakeTimeAccessor(
						&MgmpProtocol::m_hu11MeshnetMGMPrannInterval),
					MakeTimeChecker()
					)
				.AddAttribute("MaxTtl",
...
#ifndef HUMGMP_UNUSED_MY_CODE
				.AddAttribute("UnicastRannThreshold",
					"Maximum number of Rann receivers, when we send a Rann as a chain of unicasts",
					UintegerValue(1),
					MakeUintegerAccessor(
						&MgmpProtocol::m_unicastRannThreshold),
					MakeUintegerChecker<uint8_t>(1)
					)
#endif
		...
		MgmpProtocol::MgmpProtocol() :
			...
			m_maxQueueSize(255),
			m_hu11MeshnetMGMPmaxPREQretries(3),
			m_hu11MeshnetMGMPnetDiameterTraversalTime(MicroSeconds(1024 * 100)),
			m_hu11MeshnetMGMPpreqMinInterval(MicroSeconds(1024 * 100)),
			m_hu11MeshnetMGMPperrMinInterval(MicroSeconds(1024 * 100)),
			m_hu11MeshnetMGMPactiveRootTimeout(MicroSeconds(1024 * 5000)),
			m_hu11MeshnetMGMPactivePathTimeout(MicroSeconds(1024 * 5000)),
			m_hu11MeshnetMGMPpathToRootInterval(MicroSeconds(1024 * 2000)),
			m_hu11MeshnetMGMPrannInterval(MicroSeconds(1024 * 5000)),
			m_isRoot(false),
			m_maxTtl(32),
			m_unicastPerrThreshold(32),
			m_unicastPreqThreshold(1),
#ifndef HUMGMP_UNUSED_MY_CODE
			m_unicastRannThreshold(1),
#endif
		...
		bool
			MgmpProtocol::RequestRoute(
				...
				MeshnetL2RoutingProtocol::RouteReplyCallback routeReply
				)
		{
			Ptr <Packet> packet = constPacket->Copy();
			MgmpTag tag;
			if (sourceIface == GetMeshnetPoint()->GetIfIndex())
			{
				// packet from level 3
		...
		bool
			MgmpProtocol::Install(Ptr<MeshnetPointDevice> mp)
		{
			m_mp = mp;
			...
			for (std::vector<Ptr<NetDevice> >::const_iterator i = interfaces.begin(); i != interfaces.end(); i++)
			{
				...
				Ptr<MeshnetWifiInterfaceMac>  mac = wifiNetDev->GetMac()->GetObject<MeshnetWifiInterfaceMac>();
				if (mac == 0)
				{
					return false;
				}
			...
			// Meshnet point aggregates all installed protocols
			mp->AggregateObject(this);
			m_address = Mac48Address::ConvertFrom(mp->GetAddress()); // address;
			return true;
		}
		void
			MgmpProtocol::HuperLinkStatus(Mac48Address meshnetPointAddress, Mac48Address huperAddress, uint32_t interface, bool status)
		{
			if (status)
			{
				return;
			...
			MgmpProtocol::ShouldSendPreq(Mac48Address dst)
		{
			...
			if (i == m_preqTimeouts.end())
			{
				m_preqTimeouts[dst].preqTimeout = Simulator::Schedule(
					Time(m_hu11MeshnetMGMPnetDiameterTraversalTime * 2),
			...
			MgmpProtocol::RetryPathDiscovery(Mac48Address dst, uint8_t numOfRetry)
		{
			...
			if (numOfRetry > m_hu11MeshnetMGMPmaxPREQretries)
			{
				QueuedPacket packet = DequeueFirstPacketByDst(dst);
			...
			m_preqTimeouts[dst].preqTimeout = Simulator::Schedule(
				Time((2 * (numOfRetry + 1)) *  m_hu11MeshnetMGMPnetDiameterTraversalTime),
			...
			MgmpProtocol::SendProactivePreq()
		{
			...
			preq.SetLifetime(m_hu11MeshnetMGMPactiveRootTimeout.GetMicroSeconds() / 1024);
			...
			m_proactivePreqTimer = Simulator::Schedule(m_hu11MeshnetMGMPpathToRootInterval, &MgmpProtocol::SendProactivePreq, this);
		}
		bool
		...
		{
			return m_hu11MeshnetMGMPpreqMinInterval;
		}
		Time
			MgmpProtocol::GetPerrMinInterval()
		{
			return m_hu11MeshnetMGMPperrMinInterval;
		}
		uint8_t
		...
		{
			return m_hu11MeshnetMGMPactivePathTimeout.GetMicroSeconds() / 1024;
		}
		uint8_t
			...
			MgmpProtocol::Report(std::ostream & os) const
		{
			os << "<Mgmp "
				...
				"Hu11MeshnetMGMPmaxPREQretries=\"" << (uint16_t)m_hu11MeshnetMGMPmaxPREQretries << "\"" << std::endl <<
				"Hu11MeshnetMGMPnetDiameterTraversalTime=\"" << m_hu11MeshnetMGMPnetDiameterTraversalTime.GetSeconds() << "\"" << std::endl <<
				"Hu11MeshnetMGMPpreqMinInterval=\"" << m_hu11MeshnetMGMPpreqMinInterval.GetSeconds() << "\"" << std::endl <<
				"Hu11MeshnetMGMPperrMinInterval=\"" << m_hu11MeshnetMGMPperrMinInterval.GetSeconds() << "\"" << std::endl <<
				"Hu11MeshnetMGMPactiveRootTimeout=\"" << m_hu11MeshnetMGMPactiveRootTimeout.GetSeconds() << "\"" << std::endl <<
				"Hu11MeshnetMGMPactivePathTimeout=\"" << m_hu11MeshnetMGMPactivePathTimeout.GetSeconds() << "\"" << std::endl <<
				"Hu11MeshnetMGMPpathToRootInterval=\"" << m_hu11MeshnetMGMPpathToRootInterval.GetSeconds() << "\"" << std::endl <<
				"Hu11MeshnetMGMPrannInterval=\"" << m_hu11MeshnetMGMPrannInterval.GetSeconds() << "\"" << std::endl <<
...
#ifndef HUMGMP_UNUSED_MY_CODE
				"unicastRannThreshold=\"" << (uint16_t)m_unicastRannThreshold << "\"" << std::endl <<
#endif
...
#ifndef HUMGMP_UNUSED_MY_CODE
		void MgmpProtocol::SendRann(IeRann rann)
		{
			NS_LOG_DEBUG("Send Rann");
			for (MgmpProtocolMacMap::const_iterator i = m_interfaces.begin(); i != m_interfaces.end(); i++)
			{
				i->second->SendRann(rann);
			}
			m_proactivePreqTimer = Simulator::Schedule(m_hu11MeshnetMGMPpathToRootInterval, &MgmpProtocol::SendRann, this, rann);
		}
		void MgmpProtocol::ReceiveRann(IeRann rann, Mac48Address from, uint32_t interface, Mac48Address fromMp, uint32_t metric)
		{
			...
			if (i != m_mgmpSeqnoMetricDatabase.end())
			{
				if ((int32_t)(i->second.first - rann.GetOriginatorSeqNumber()) > 0)
				{
					return;
				}
				if (i->second.first == rann.GetOriginatorSeqNumber())
				{
					freshInfo = false;
					if (i->second.second <= rann.GetMetric())
					{
						return;
					}
				}
			...
			if (freshInfo || m_rtable->LookupReactive(rann.GetOriginatorAddress()).metric > rann.GetMetric())
			{
				...
				ReactivePathResolved(rann.GetOriginatorAddress());
			}
			if (m_rtable->LookupReactive(fromMp).metric > metric)
			{
				m_rtable->AddReactivePath(
					fromMp,
					from,
		...
		std::vector<Mac48Address> MgmpProtocol::GetRannReceivers(uint32_t interface)
		{
			...
			if ((retval.size() >= m_unicastRannThreshold) || (retval.size() == 0))
			{
				retval.clear();
				retval.push_back(Mac48Address::GetBroadcast());
			}
