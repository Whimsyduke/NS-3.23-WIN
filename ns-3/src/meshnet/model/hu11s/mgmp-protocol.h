...
namespace ns3 {
	...
	namespace hu11s {
		 ...
		 * \ingroup hu11s
		 *
		 * \brief Hybrid wireless meshnet protocol -- a routing protocol of IEEE 802.11s draft.
		 */
		class MgmpProtocol : public MeshnetL2RoutingProtocol
		{
		public:
			...
			/// Route request, inherited from MeshnetL2RoutingProtocol
			...
			/**
			 * \brief Install MGMP on given meshnet point.
			 *
			 * Installing protocol cause installing its interface MAC plugins.
			 *
			 * Also MP aggregates all installed protocols, MGMP protocol can be accessed
			 * via MeshnetPointDevice::GetObject<hu11s::MgmpProtocol>();
			 */
			bool Install(Ptr<MeshnetPointDevice>);
			void HuperLinkStatus(Mac48Address meshnetPontAddress, Mac48Address huperAddress, uint32_t interface, bool status);
		...
		private:
			...
			struct QueuedPacket
			{
				...
				uint32_t inInterface; ///< incoming device interface ID. (if packet has come from upper layers, this is Meshnet point ID)
			...
			///\}
			///\return address of MeshnetPointDevice
			...
			///\}
#ifndef HUMGMP_UNUSED_MY_CODE
			void SendRann(IeRann rann);
			void ReceiveRann(IeRann rann, Mac48Address from, uint32_t interface, Mac48Address fromMp, uint32_t metric);
#endif
		private:
			///\name Statistics:
			///\{
			...
			uint8_t m_hu11MeshnetMGMPmaxPREQretries;
			Time m_hu11MeshnetMGMPnetDiameterTraversalTime;
			Time m_hu11MeshnetMGMPpreqMinInterval;
			Time m_hu11MeshnetMGMPperrMinInterval;
			Time m_hu11MeshnetMGMPactiveRootTimeout;
			Time m_hu11MeshnetMGMPactivePathTimeout;
			Time m_hu11MeshnetMGMPpathToRootInterval;
			Time m_hu11MeshnetMGMPrannInterval;
			bool m_isRoot;
			uint8_t m_maxTtl;
			uint8_t m_unicastPerrThreshold;
			uint8_t m_unicastPreqThreshold;
#ifndef HUMGMP_UNUSED_MY_CODE
			uint8_t m_unicastRannThreshold;
#endif
